...
import org.homonoia.sw.assets.loaders.oddl.primitives.UnsignedInt16Structure;
import org.homonoia.sw.assets.loaders.oddl.primitives.UnsignedInt32Structure;
import org.homonoia.sw.assets.loaders.oddl.primitives.UnsignedInt64Structure;
import org.homonoia.sw.assets.loaders.oddl.primitives.UnsignedInt8Structure;
...
public abstract class NodeStructure extends Structure implements Iterable<Structure> {
        ...
        for (final Map.Entry<String, String> property : properties.entrySet()) {
            ...
            if (value.charAt(0) == '$' || value.charAt(0) == '%') {
                ...
                if (reference.isGlobal()) {
                    rootStructure.getStructure(reference)
                            .orElseThrow(() -> new OpenDDLException("Unable to resolve global reference " + reference));
                } else {
                    NodeStructure ancestor = getParentStructure();

                    while (ancestor != rootStructure) {
                        if (ancestor.getStructure(reference).isPresent()) {
                            break;
                        }
    ...
    public final Optional<Structure> getStructure(final Reference reference) {
        return getStructure(reference, 0);
    }

    public final Optional<Structure> getFirstStructure(final String identifier) {
        for (final Structure structure : structures) {
    ...
    public final Optional<Structure> getLastStructure(final String identifier) {
    ...
    public final int getStructureCount(final String identifier) {
        return Math.toIntExact(structures.stream()
                .map(Structure::getStructureIdentifier)
    ...
    public final boolean getBooleanProperty(final String key) throws OpenDDLException {
        return Decoder.decodeBoolean(properties.get(key));
    }
    ...
    private class StructureIdentifierIterator implements Iterator<Structure> {
        ...
        private final Iterator<Structure> iterator;
        ...
        public StructureIdentifierIterator(final String identifier) {
            ...
            this.iterator = structures.iterator();
